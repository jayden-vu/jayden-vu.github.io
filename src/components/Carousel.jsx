...
export default function Carousel({ items = DEFAULT_ITEMS, baseWidth = 300, autoplay = false, autoplayDelay = 3000, pauseOnHover = false, loop = false, round = false }) {
  ...
  const renderIcon = item => {
    ...
    if (item.image) {
      return (
        <div className="h-[40px] w-[40px] rounded-full overflow-hidden bg-gray-700 flex items-center justify-center">
          ...
          />
          <div className="h-full w-full hidden items-center justify-center text-white text-lg font-bold bg-gray-600">{item.title.charAt(0)}</div>
        </div>
      );
    }
    ...
    if (typeof item.icon === 'string') {
      return <span className="flex h-[40px] w-[40px] items-center justify-center rounded-full bg-[#060010] text-2xl">{item.icon}</span>;
    }

    // Default React icon component
    return <span className="flex h-[40px] w-[40px] items-center justify-center rounded-full bg-[#060010]">{item.icon}</span>;
  };
        ...
        {carouselItems.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              transition={effectiveTransition}>
              <div className={`${round ? 'p-0 m-0' : 'mb-4 p-5'} ${round ? '' : 'flex-shrink-0'}`}>{renderIcon(item)}</div>
