...
const experiences = [
  {
    ...
    period: '2021-2022',
    description: ['Joined as a software intern in the Tool team', 'Learned objective-C, and the Sketch plugin lifecycle', 'Build a desktop app using Reactotron', 'Participate in algorithm competitions'],
  },
  {
    id: 2,
    company: 'Step up',
    ...
    period: '2022 - 2023',
    description: ['Achieved 1M+ downloads with 4.85/5 star rating', 'Integrated Jitsi for video/audio calls with performance optimization', 'Developed Mini App Club for social chatting and gaming', 'Integrated multiple payment methods (Momo, Apple Pay, Google Play)', 'Conducted A/B Testing and Alpha Testing for feature optimization'],
  },
  {
    id: 3,
    company: 'AIT',
    ...
    description: [
      ...
      'Work on multiple projects with clients from different industries',
      'Optimized application performance with callback, memo techniques',
      'Integrate SDKs from partners',
      'Published reusable libraries on NPM',
      'Ability to self-learn and update the latest current technologies',
      'Employee of the Year (2023-2025)',
      //
    ],
  },
];
...
const Experience = () => {
  ...
  useEffect(() => {
    const handleScroll = e => {
      ...
      if (e.type === 'wheel') {
        ...
        if (currentIndex === experiences.length - 1 && scrollDown && touchZone.bottom > screenHeight - 100) {
  ...
  const ExperienceCard = ({ exp, isActive, direction }) => (
    <AnimatePresence mode="wait">
      {isActive && (
        <motion.div key={exp.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 0.4, ease: 'easeOut' }} className="w-full max-w-5xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row lg:gap-16 items-center lg:items-start">
            ...
            <div className="lg:w-1/3 flex flex-col items-center lg:items-end mb-8 lg:mb-0">
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-center lg:text-right">
                <motion.div
                ...
                />
                {/* Company */}
                <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.5 }} className="mt-2 text-lg md:text-xl font-medium text-blue-400">
                  {exp.company}
                </motion.p>
              </motion.div>
            </div>
            ...
            <div className="lg:w-2/3">
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="space-y-6">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0.3 }}>
                  <span className="inline-block px-3 py-1 text-sm font-medium text-gray-300 bg-white/10 rounded-full border border-white/20">{exp.duration}</span>
                </motion.div>
                ...
                {/* Description */}
                <motion.div className="space-y-3 pt-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4, delay: 0.6 }}>
                  {exp.description.map((point, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.7 + i * 0.1 }} className="flex items-start gap-3">
                      <div
  ...
  return (
    <section ref={containerRef} id="experience" className="relative bg-black text-white w-full overflow-hidden min-h-screen" style={{ fontFamily: "'SF Pro Display', 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      <ExperienceAnimatedBackground className="z-0" />
      <div className="relative z-10 h-screen flex flex-col">
        {/* Header */}
        <div className="pt-20 pb-10 flex-shrink-0">
          <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, amount: 0.3 }} transition={{ duration: 1, ease: 'easeOut' }} className="text-center px-6">
            ...
            <p className="text-white/60 max-w-2xl mx-auto text-lg md:text-xl">My professional journey in software development</p>
          </motion.div>
        </div>
        ...
        <div className="flex-1 flex items-start justify-center min-h-0 ">
          {isInExperience ? (
            <div className="w-full" style={{ minHeight: '60vh' }}>
              <ExperienceCard exp={experiences[currentIndex]} isActive={true} direction={scrollDirection} />
            </div>
          ) : (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-white/60 flex-1 flex items-center justify-center">
        ...
        {/* <div className="flex-shrink-0 pb-8">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.5 }} className="flex justify-center items-center gap-4">
            <div className="flex items-center gap-2 bg-black/30 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
              ...
              <motion.div animate={{ y: [0, 3, 0], opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }} className="w-1.5 h-1.5 rounded-full bg-white/60" />
            </div>
