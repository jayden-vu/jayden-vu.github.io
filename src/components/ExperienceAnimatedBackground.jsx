...
const ExperienceAnimatedBackground = ({ className = '' }) => {
  ...
  const FloatingShape = ({ delay = 0, duration = 20, startX = 0, startY = 0, shape = 'circle' }) => (
    <motion.div
      ...
      {shape === 'circle' ? <div className="w-4 h-4 rounded-full bg-gradient-to-br from-orange-400/40 to-purple-500/30 shadow-lg backdrop-blur-sm border border-white/10" /> : shape === 'square' ? <div className="w-3 h-3 bg-gradient-to-br from-purple-400/40 to-pink-500/30 shadow-lg backdrop-blur-sm border border-white/10 rotate-45" /> : <div className="w-0 h-0 border-l-2 border-r-2 border-b-3 border-transparent border-b-orange-400/40 shadow-lg" />}
    </motion.div>
  );
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden ${className}`}>
      ...
      <motion.div
        ...
        animate={{
          background: ['linear-gradient(to bottom, transparent 0%, rgba(199,21,133,0.25) 50%, transparent 100%)', 'linear-gradient(to bottom, transparent 0%, rgba(147,51,234,0.20) 50%, transparent 100%)', 'linear-gradient(to bottom, transparent 0%, rgba(168,85,247,0.30) 50%, transparent 100%)', 'linear-gradient(to bottom, transparent 0%, rgba(199,21,133,0.25) 50%, transparent 100%)'],
        }}
      ...
      <motion.div
        ...
        animate={{
          background: ['linear-gradient(to bottom, transparent 0%, rgba(147,51,234,0.20) 50%, transparent 100%)', 'linear-gradient(to bottom, transparent 0%, rgba(168,85,247,0.25) 50%, transparent 100%)', 'linear-gradient(to bottom, transparent 0%, rgba(199,21,133,0.22) 50%, transparent 100%)', 'linear-gradient(to bottom, transparent 0%, rgba(147,51,234,0.20) 50%, transparent 100%)'],
        }}
