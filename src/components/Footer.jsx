...
import { FiArrowUp, FiCoffee, FiDownload, FiExternalLink, FiGithub, FiHeart, FiLinkedin, FiMail, FiMapPin, FiSend } from 'react-icons/fi';

const Footer = () => {
  const currentYear = new Date().getFullYear();

  const socialLinks = [
    { icon: FiGithub, href: 'https://github.com/jayden-vu', label: 'GitHub', color: '#ffffff' },
    { icon: FiLinkedin, href: 'https://www.linkedin.com/in/jayden-vu/', label: 'LinkedIn', color: '#0077b5' },
    ...
    { icon: FiSend, href: 'https://t.me/jaydenvu124', label: 'Telegram', color: '#0088cc' },
    { icon: FiMail, href: 'mailto:jayden.vu.work@gmail.com', label: 'Email', color: '#ea4335' },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-2">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8 }}>
              <div className="mb-6">
                <h3 className="text-2xl font-bold bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">Jayden Vu</h3>
                <div className="mt-2">
                  <p className="text-sm text-blue-400/80">{'React Native Developer'}</p>
                </div>
              </div>
              <p className="text-white/70 leading-relaxed mb-6 max-w-md">{'Passionate software engineer crafting digital experiences with clean code and innovative solutions. Always learning, always building something amazing.'}</p>
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, delay: 0.1 }}>
              <h4 className="font-semibold mb-6 text-white/90 text-lg">Quick Links</h4>
              <ul className="space-y-4">
                {quickLinks.map((link, index) => (
                  <li key={index}>
                    <motion.a href={link.href} className="text-white/60 hover:text-white text-sm transition-all duration-300 flex items-center gap-3 group" whileHover={{ x: 5 }}>
                      <motion.span className="w-1.5 h-1.5 bg-white/40 rounded-full group-hover:bg-blue-400 transition-all duration-300" whileHover={{ scale: 1.5 }} />
                      {link.name}
                    </motion.a>
                  </li>
                ))}
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false }} transition={{ duration: 0.8, delay: 0.2 }}>
              ...
              <div className="space-y-3">
                <motion.a
                  href="mailto:jayden.vu.work@gmail.com"
                ...
                <motion.a
                  href={`${import.meta.env.BASE_URL}/resume/jayden_vu_resume.pdf`}
                  download
                  className="flex items-center gap-3 p-3 rounded-lg text-white/70 hover:text-white text-sm transition-all duration-300 group"
                  style={{
                    background: 'linear-gradient(135deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%)',
      ...
      <div className="relative z-10 border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            {/* Enhanced Copyright */}
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }} transition={{ duration: 0.8 }} className="flex items-center gap-2 text-white/60 text-sm">
              ...
              <span>{'by Jayden Vu'}</span>
            </motion.div>

            {/* Enhanced Tech Stack */}
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: false }} transition={{ duration: 0.8, delay: 0.2 }} className="flex items-center gap-4 text-white/50 text-xs">
              {['React Native', 'Expo', 'Reanimated', 'Tailwind CSS', 'Vercel'].map((e, idx, arr) => {
                return (
                  <>
                    <span className="hover:text-white/70 transition-colors cursor-default">{e}</span>
