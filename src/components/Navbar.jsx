    ...
    <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[96%] max-w-7xl transition-all duration-500 ease-in-out">
      ...
      <div className="relative backdrop-filter backdrop-blur-sm bg-black/10 border border-white/20 rounded-2xl shadow-2xl">
        ...
        <div className="relative px-6 py-4 flex justify-between items-center font-[Inter,sans-serif]">
          ...
          <button onClick={toggleMenu} className="md:hidden relative flex flex-col justify-center items-center w-10 h-10 space-y-1.5 focus:outline-none backdrop-blur-xl bg-white/10 border border-white/20 rounded-xl transition-all duration-300 hover:bg-cyan-300/20" aria-label="Toggle menu">
            <span
              className={`transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2' : ''}`}
              style={{
                width: '20px',
                height: '2px',
      ...
      <div className={`md:hidden absolute top-full left-0 w-full mt-2 transition-all duration-500 ease-in-out ${isMenuOpen ? 'opacity-100 translate-y-0 visible' : 'opacity-0 -translate-y-4 invisible'}`}>
