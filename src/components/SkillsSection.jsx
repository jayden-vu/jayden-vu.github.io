...
import { FiBarChart2, FiCloud, FiCode, FiCpu, FiDatabase, FiDollarSign, FiGitBranch, FiHardDrive, FiLayers, FiMonitor, FiServer, FiSettings, FiShield, FiTarget, FiTool, FiWifi, FiZap } from 'react-icons/fi';
  ...
  mainSkills: [
    {
      ...
      skills: ['Javascript & Typescript', 'Nodejs', 'Java (Android)', 'Swift (iOS)', 'Native Modules (new & old)', 'Nextjs & Nestjs'],
      color: '#ff6b35',
      proficiencyLevel: 'Advanced', // Learning, Beginner, Intermediate, Advanced, Expert
    },
    {
      icon: <FiDollarSign />,
      title: 'ReactNative & Expo',
      skills: ['Performance Optimization', 'Reanimated', 'Skia', 'Chart', 'Svg', 'Tool Other'],
      color: '#00ff88',
    ...
    },
    {
      ...
      skills: ['Firebase', 'REST APIs', 'Socket.IO', 'GraphQL', 'Supabase', 'MySQL', 'MongoDB'],
      color: '#4ecdc4',
      proficiencyLevel: 'Intermediate',
    },
    {
      ...
      skills: ['Git', 'GitHub', 'CI/CD Pipelines', 'Flatlane', 'OTA', 'NPM Publishing'],
      color: '#00ffff',
      proficiencyLevel: 'Intermediate',
    },
    {
      ...
      skills: ['DevOps Tools', 'Kubernetes'],
      color: '#ffaa00',
...
const SkillsSection = () => {
  ...
  const CircuitPattern = ({ delay = 0, startX = 0, startY = 0, color = '#00ffff' }) => (
    <motion.div
      ...
      <svg width="60" height="60" viewBox="0 0 60 60" fill="none">
        <path d="M10 10h15v15M45 10h-15v15M10 50h15v-15M45 50h-15v-15M25 25h10v10" stroke={color} strokeWidth="1.5" opacity="0.7" />
  ...
  const techIcons = [<FiCode />, <FiDatabase />, <FiGitBranch />, <FiSettings />, <FiBarChart2 />, <FiCpu />, <FiServer />, <FiMonitor />, <FiHardDrive />, <FiWifi />, <FiLayers />, <FiCloud />, <FiShield />, <FiTool />, <FiTarget />];
    ...
    <motion.div
      ...
      {skills.map((panel, idx) => (
        <motion.div
          ...
          <SkillsPanel {...panel} expandRatio={isMobile ? 1 : animation.expandRatio} transitionDuration={animation.transitionDuration} proficiencyLevel={panel.proficiencyLevel || 'Expert'} isMobile={isMobile} />
        </motion.div>
      ))}
    </motion.div>
  );
  ...
  return (
    <section id="skills" className="relative bg-black text-white py-20 px-6 md:px-12 lg:px-20 w-full overflow-hidden min-h-screen" style={{ fontFamily: "'SF Pro Display', 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <motion.div
          ...
          animate={{
            background: ['radial-gradient(ellipse 80% 60% at 50% 50%, rgba(0,255,255,0.06) 0%, transparent 60%)', 'radial-gradient(ellipse 85% 65% at 50% 50%, rgba(255,107,53,0.05) 0%, transparent 60%)', 'radial-gradient(ellipse 90% 70% at 50% 50%, rgba(78,205,196,0.07) 0%, transparent 60%)', 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(0,255,255,0.06) 0%, transparent 60%)'],
          }}
          transition={{
            duration: 8,
            repeat: Infinity,
        ...
        {particles.map((particle, index) => (
          <FloatingParticle key={`particle-${index}`} delay={particle.delay} startX={particle.startX} startY={particle.startY} color={particle.color} size={particle.size} />
        ))}

        {/* Background tech icons with more variety and better visibility */}
        {backgroundIcons.map((iconData, index) => (
          <BackgroundIcon key={`bg-icon-${index}`} delay={iconData.delay} startX={iconData.startX} startY={iconData.startY} icon={iconData.icon} color={iconData.color} />
        ))}

        {/* Enhanced circuit patterns for tech feel */}
        {circuitPatterns.map((pattern, index) => (
          <CircuitPattern key={`circuit-${index}`} delay={pattern.delay} startX={pattern.startX} startY={pattern.startY} color={pattern.color} />
        ))}
        ...
        <motion.div
          ...
          animate={{
            background: ['linear-gradient(90deg, transparent 0%, rgba(0,255,255,0.4) 50%, transparent 100%)', 'linear-gradient(90deg, transparent 0%, rgba(255,107,53,0.35) 50%, transparent 100%)', 'linear-gradient(90deg, transparent 0%, rgba(78,205,196,0.4) 50%, transparent 100%)'],
          }}
        ...
        <motion.div
          ...
          animate={{
            background: ['linear-gradient(90deg, transparent 0%, rgba(78,205,196,0.4) 50%, transparent 100%)', 'linear-gradient(90deg, transparent 0%, rgba(0,255,255,0.4) 50%, transparent 100%)', 'linear-gradient(90deg, transparent 0%, rgba(255,107,53,0.35) 50%, transparent 100%)'],
          }}
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: false, amount: 0.3 }} transition={{ duration: 0.8 }} className="relative z-10">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.5 }} className="flex justify-center mt-12 gap-4">
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.8 }} className="text-center mt-8">
